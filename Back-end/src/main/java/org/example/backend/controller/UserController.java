...
public class UserController {
        ...
        switch (res) {
            case VarList.OK -> {
                return ResponseEntity.status(HttpStatus.OK)
                        .body(new ResponseDTO(VarList.OK, "Success", updateUserDTO));
            }
   ...
   public ResponseEntity<ResponseDTO> deleteUser(@PathVariable long id, @RequestHeader("Authorization") @Valid String authHeader) {
    try {
   ...
   public ResponseEntity<ResponseDTO> getUser( long id, @RequestHeader("Authorization") @Valid String authHeader) {
   ...
   @GetMapping(value = "/get-all")
    public ResponseEntity<ResponseDTO> getAllUsers(@RequestHeader("Authorization") @Valid String authHeader) {
       System.out.println("method triggered");
       try {
            List<UserDTO> userDTOList = userService.getAllUsers();
            return ResponseEntity.status(HttpStatus.OK)
                    .body(new ResponseDTO(VarList.OK, "Success", userDTOList));
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(new ResponseDTO(VarList.Internal_Server_Error, e.getMessage(), null));
        }
